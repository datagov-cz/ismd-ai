...
class Universal(BaseModel):
    ...
    id: str = Field(description="Unique application identifier for the entity")
...
class ClassReference(BaseModel):
    ...
    id: str = Field(description="Unique identifier of the referenced class")
...
class Class(Universal):
    ...
    ownsAttribute: Optional[List[Attribute]] = Field(default=None, description="List of attributes owned by this class")
    specializes: Optional[List[ClassReference]] = Field(default=None, description="List of references to the parent classes that this class specializes")
...
class Relationship(Universal):
    ...
    mediatesClass: Optional[List[ClassReference]] = Field(default=None, description="List of classes that this relationship mediates between. Only binary relationships mediating two classes are considered in the current implementation, but this can be extended to support n-ary relationships in the future.")

class ConceptualModel(BaseModel):
    """Represents a conceptual model consisting of classes and relationships."""
    classes: List[Class] = Field(default_factory=list, description="List of classes in the conceptual model")
    relationships: List[Relationship] = Field(default_factory=list, description="List of relationships in the conceptual model")