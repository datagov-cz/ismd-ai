...
- `api/routers`: FastAPI route definitions and HTTP error handling
- `api/schemas`: request and response models exposed by the REST API
- `api/mappers`: translation between API schemas and internal domain models
- `application/services`: business workflows such as authentication, job lifecycle, feedback recording, token limiting, and legal-text selection
- `application/executors`: thin orchestration adapters between services and LLM generators
- `domain/models`: internal legal-act, conceptual-model, language, and suggestion data objects
- `infrastructure/legal_acts`: eSbirka/ESEL legal-act loading
- `infrastructure/llm`: LLM generators, prompt construction, generator ports, and structured LLM output schemas
- `infrastructure/persistence`: filesystem/JSONL persistence for jobs, suggestions, and suggestion evaluations
